      ...
      <div className="flex">
        <div className="w-80 border-r border-gray-800 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 73px)' }}>
          <div className="p-3 space-y-1">
            {activities.map((a, i) => (
              <button key={i} onClick={() => a.metadata?.traceId && loadTrace(a.metadata.traceId)} className={"w-full text-left px-3 py-2 rounded-lg text-sm hover:bg-gray-800 transition-colors " + (selectedTraceId === a.metadata?.traceId ? 'bg-gray-800' : '')}>
                ...
                <div className="text-xs text-gray-600 mt-0.5 ml-4">{a.agentId ? a.agentId.split('.').pop() : 'system'}</div>
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-6" style={{ maxHeight: 'calc(100vh - 73px)' }}>
          ...
          {trace && (
            <div className="space-y-4">
              ...
              {explanation && <div className="bg-gray-900 border border-gray-800 rounded-lg p-4"><h3 className="text-sm font-medium text-gray-400 mb-2">Explanation</h3><p className="text-sm text-gray-300 whitespace-pre-line">{explanation}</p></div>}
              <h3 className="text-sm font-medium text-gray-400">Span Waterfall ({trace.spans.length} steps)</h3>
              <div className="space-y-1">{trace.spans.map((s, i) => {
                const dur = s.startedAt && s.endedAt ? Math.round((new Date(s.endedAt).getTime() - new Date(s.startedAt).getTime())) : 0;
                return (
                  <div key={i} className={"flex items-center gap-3 px-3 py-2 rounded-lg " + (s.spanType === 'tool_call' ? 'ml-8' : s.spanType === 'handoff' ? 'ml-4' : '')}>
