...
const memTypeColors: Record<string, string> = { founder_preference: "bg-blue-500/20 text-blue-400", business_fact: "bg-emerald-500/20 text-emerald-400", decision_log: "bg-purple-500/20 text-purple-400", relationship_context: "bg-cyan-500/20 text-cyan-400", strategic_goal: "bg-amber-500/20 text-amber-400", constraint: "bg-red-500/20 text-red-400" };
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="p-6">
        ...
        {tab === "traces" && !selectedTrace && (
          <div>
            <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold">Traces</h2><button onClick={loadTraces} className="px-3 py-1.5 bg-gray-800 rounded text-sm hover:bg-gray-700">Refresh</button></div>
        ...
        {tab === "traces" && selectedTrace && (
          <div>
            <button onClick={() => setSelectedTrace(null)} className="text-blue-400 text-sm mb-4 hover:underline">&larr; Back to traces</button>
            ...
            <div className="space-y-1">{selectedTrace.spans.map((s, i) => {
              const dur = s.startedAt && s.endedAt ? new Date(s.endedAt!).getTime() - new Date(s.startedAt).getTime() : 0;
              return (
                <div key={i} className={"flex items-center gap-3 px-3 py-2 rounded " + (s.spanType === "tool_call" ? "ml-6" : "")}>
        ...
        {tab === "cost" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">{layerMetrics.map((l, i) => (
              <div key={i} className="bg-gray-900 rounded-lg border border-gray-800 p-4">
                <div className="text-xs text-gray-500 mb-1">{l.layer}</div><div className="text-xl font-bold">${l.cost.toFixed(2)}</div>
        ...
        {tab === "leaderboard" && (
          <div><h2 className="text-lg font-semibold mb-4">Agent Leaderboard (7 days)</h2>
              ...
              <div className="overflow-x-auto"><table className="w-full text-sm">
                ...
                <tbody>{leaderboard.map((e, i) => (
                  <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-900/50">
                    ...
                    <td className="py-3 pr-4">${e.weeklyCost.toFixed(2)}</td><td className="py-3 pr-4">{e.totalTasks}</td><td className="py-3 pr-4 text-red-400">{e.failures}</td><td className="py-3">{e.escalationRate}%</td>
                  </tr>
                ))}</tbody>
              </table></div>
            )}
        ...
        {tab === "evals" && (
          <div><h2 className="text-lg font-semibold mb-4">Evaluation Dashboard</h2>
            {evalHistory.length === 0 ? <div className="text-gray-500 bg-gray-900 rounded-lg p-6 text-center">No eval runs yet.</div> : (
              <div className="overflow-x-auto"><table className="w-full text-sm">
                ...
                <tbody>{evalHistory.map((e, i) => (
                  <tr key={i} className={"border-b border-gray-800/50 " + (!e.passed ? "bg-red-500/5" : "")}>
                    ...
                    <td className="py-3 pr-4 text-emerald-400">{e.passedTests}</td><td className="py-3 pr-4 text-red-400">{e.failedTests}</td>
                    <td className="py-3 pr-4 text-xs text-gray-500">{e.triggeredBy}</td><td className="py-3 text-xs text-gray-500">{new Date(e.createdAt).toLocaleString()}</td>
                  </tr>
                ))}</tbody>
              </table></div>
            )}
        ...
        {tab === "memory" && (
          <div>
            ...
            {memories.length === 0 ? <div className="text-gray-500 bg-gray-900 rounded-lg p-6 text-center">No memories stored yet. Memories are created from founder statements, approval decisions, and agent pattern detection.</div> : (
              <div className="space-y-2">{memories.map((m, i) => (
