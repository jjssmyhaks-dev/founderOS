...
export class ResearchLayerService {
  ...
  private async dispatch(founderId: string, agentId: string, goal: string, routing: any): Promise<any> {
    ...
    await this.prisma.task.create({
      data: {
        ...
        description: goal,
        goal,
        triggerType: 'orchestrator_assigned',
        status: 'PENDING',
  ...
  async handleMessage(founderId: string, message: string, routing: any) {
    ...
    if (lower.includes('competitor') || lower.includes('rival')) return this.dispatch(founderId, 'competitor-intelligence', message, routing);
    if (lower.includes('market') || lower.includes('trend')) return this.dispatch(founderId, 'market-trend-scanning', message, routing);
    if (lower.includes('pricing') || lower.includes('price')) return this.dispatch(founderId, 'pricing-benchmarking', message, routing);
    if (lower.includes('customer') || lower.includes('audience')) return this.dispatch(founderId, 'customer-audience-research', message, routing);
    return this.dispatch(founderId, 'campaign-deep-dive', message, routing);
  }
